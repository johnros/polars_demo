...
__Careful__: `over()` should follow computing expression. The following will not fail, but return the wrong result: 
```{python}
df.with_columns(
...
What if you want to access a column from frame `df`, when processing frame `df2`?
This is what `df.with_context()` will do.
...
At the time of writing, if you give polars a remote glob, it will only read the first file ([ref](https://github.com/pola-rs/polars/issues/5863)). I expect this to change in the near future.
...
See [here](https://www.rhosignal.com/posts/polars-aws-lambda/) for working in serverless environments. 
...
__Interesting__:
How do I know if a python function can take a polars frame as input?
