...
```{python}
#| label: polars-lazy-wrong-order-optimization-plan-2
...
(
    df
    .melt(value_vars=['a', 'b']) # ordering here is important
    .sort('value')
    .select(
        value=col('value').filter(col('variable') == 'b'),
        proportion_lte=(
            col('variable').eq('a').cum_sum()
            .filter(col('variable') == 'b')
            / df.height
        )
    )
)
...
q = (
    ...
    .with_columns(
        pl.col('float').mapping(remapping={4.0:None}, default=100).fill_null(pl.col('blah').mean()).alias('float2'),
        )
    )
...
-   Why not use `pl.col('blah').mean()` within the `mapping()`? That is indeed more reasonable. It simply did not work.
